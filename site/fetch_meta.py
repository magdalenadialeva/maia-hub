...
def fetch_thumbs(acct: str, token: str, want_names: set) -> Dict[str, str]:
    ...
    fields = "name,effective_status,creative{thumbnail_url,image_url}"
    # Activos Y pausados recientes: así los creativos que se muestran en el hub
    # (que muchas veces ya están pausados) también traen su miniatura real.
    filt = json.dumps([{"field": "ad.effective_status", "operator": "IN",
                        "value": ["ACTIVE", "PAUSED", "ADSET_PAUSED",
    ...
    thumbs: Dict[str, str] = {}
    statuses: Dict[str, str] = {}   # nombre -> effective_status de Meta (ACTIVE/PAUSED/…)
    seen = 0
    withthumb = 0
    sample = None
    page = 0
    ...
    # Diagnóstico: cuántos anuncios, cuántos con imagen, y un nombre de muestra.
    print(f"  · thumbs diag: anuncios={seen} con_img={withthumb} muestra='{sample}'")
    return thumbs, statuses
